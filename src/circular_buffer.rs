...
impl<const N: usize> CircularBuffer<N> {
	...
	pub fn write_str(&mut self, str: &CStr) {
		// ensure the string can fit in the buffer at all (with null terminator)
...
mod tests {
	...
	fn can_write_one_string_no_wrap() {
		...
		assert_eq!(cb.start_ptr, 1);
		assert_eq!(cb.end_ptr, 6);
		assert_eq!(cb.into_iter().collect::<Vec<_>>(), vec![String::from("hello")])
	}
	...
	fn can_write_two_strings_no_wrap() {
		...
		assert_eq!(cb.into_iter().collect::<Vec<_>>(), vec![String::from("hello"), String::from("world")])
	}
}